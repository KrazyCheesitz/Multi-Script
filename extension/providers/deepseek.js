...
const ZSProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match Multi-Script's own injected
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole Multi-Script bar INTO the
  ...
  function enforceComposer(reason) {
    ...
    // off) without Multi-Script reverting their choice every frame.
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…Multi-Script: result truncated to fit DeepSeek's ${SEND_CAP}-character ` +
  ...
  //   isStarted():bool        - a Multi-Script session exists in this chat
  ...
  function findToolBlockSpot(item, chip) {
    ...
    const isNativeMarkup = (t) => P.DSML_RE.test(t);
    ...
    for (const container of containers) {
      ...
      while (i < kids.length) {
        ...
        const startsNative = isNativeMarkup(txt);
        if (!startsBlock && !isJson(txt) && !startsNative) { i++; continue; }
        // Found the start of a tool block. Hide this child…
        const runStart = i;
        let runEnd = i;
        if (startsNative) {
          // Native markup may have no separately detectable end marker to look for - the block IS the run of
          // consecutive paragraphs carrying the tags, so walk while they match.
          let j = i + 1;
          while (j < kids.length && isNativeMarkup(kids[j].textContent || "")) { runEnd = j; j++; }
