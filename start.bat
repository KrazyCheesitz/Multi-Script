...
chcp 65001 >nul
title Multi-Script Multi-Engine Bridge
cd /d "%~dp0"
...
echo.
echo   === Multi-Script Multi-Engine Bridge ===
echo.
...
if not exist "%~dp0runtime\bridge.py" (
    echo   ERROR: runtime\bridge.py not found.
    echo.
    ...
    echo.
    call :log "FATAL: runtime\bridge.py missing (run from inside ZIP?)."
    pause
    exit /b 1
)
...
echo  ##                                                        ##
echo  ##   Multi-Script stops working if you close it. Just       ##
echo  ##   minimize this window and leave it running.           ##
echo  ##                                                        ##
echo  ############################################################
echo.
call :log "Launching runtime\bridge.py with %PY%"
%PY% "%~dp0runtime\bridge.py"
...
call :log "runtime\bridge.py exited with code %BRIDGE_EXIT%"
