...
m=M();assert len(b.BUILTIN_TOOLS)==151 and len(b.STUDIO_DIRECT_CONTRACTS)==47
for req,eng,expected in [('make a beautiful animated Roblox combat system','roblox','ms_roblox_animation_studio'),('polish this shop GUI','unity','ms_unity_ui_studio'),('make a professional character model','blender','ms_blender_model_studio')]:
 d=json.loads(b._builtin_call('ms_studio_director',{'request':req,'engine':eng},m)['text']);assert d['hiddenAugmentation'] and d['neverSubstitutePlanForDeliverable'] and expected in d['selectedDirectTools'] and d['selectedSkills']
listing=json.loads(b._builtin_call('ms_list_direct_tools',{'category':'animation','limit':100},m)['text']);assert listing['totalMatches']==12
...
print('PASS studio quality autopilot: animation, UI, modeling, discovery and detailed craft contracts')
