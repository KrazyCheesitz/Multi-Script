...
The automated matrix executes four supported command envelopes for each of 151 direct Multi-Script tools on every provider contract, then tests four namespaced MCP probes per provider. This is 5,472 parser/tool-routing envelopes, in addition to 5,000 malformed-input fuzz cases, 1,000 generic round trips, provider adapter loading, bridge handler coverage, and engine-specific tests.
